...
const Hero = ({ id, className }) => {
  ...
  useEffect(() => {
    ...
    const typeText = () => {
      if (i < fullGreeting.length) {
        ...
        // Greeting is fully typed, show laptop and start typing one-liner
  ...
  return (
    <div id={id} className={`h-[91vh] flex border-black border-b-4 w-screen ${className}`}>
      <div className="border-r-2 border-black w-1/2 bg-lime-200 flex flex-col justify-between items-start font-jetbrains gap-24 px-10 pt-20 pb-0">
        <div className="flex flex-col gap-12">
          <div className="relative">
            <h1 className="text-6xl font-extrabold text-start invisible">{fullGreeting}</h1>
            <h1 className="absolute top-0 left-0 text-6xl font-extrabold text-start">
              <span id="greeting"></span>
              <span className="cursor">|</span>
            </h1>
          </div>
          <div className="relative text-right">
            <h2 className="text-3xl text-gray-800 invisible">{fullOneLiner}</h2>
            <h2 className="absolute top-0 right-0 text-3xl text-gray-800">
